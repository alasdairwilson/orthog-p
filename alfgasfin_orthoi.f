...
C calculates a new temperature based on density fluctuations.
       do l = 1,lt
	do m =  1,mt
	tt = chi*((ee(l,m,1)+dd(l,m,1))*0.5*pdense)**(2.0/3.0)

	ir = ionrt(tt)

	ioni = ir*(pdense)*ee(l,m,1)*gdense*dd(l,m,1)*tstep
	reco = rr*(pdense**2)*(ee(l,m,1)**2)*tstep
	if((l.eq.30).and.(m.eq.30))print*,ee(l,m,1),dd(l,m,1),
     & (ioni-reco)/pdense,tt

C add alfven ionisation stuff here, for now NOT ENERGY CONSERVING
	
	ee(l,m,1)= (ee(l,m,1)*pdense+ioni-reco)/pdense
	dd(l,m,1)= (dd(l,m,1)*gdense-ioni+reco)/gdense
