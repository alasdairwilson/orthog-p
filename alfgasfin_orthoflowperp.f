...
* opens all i/o files, reads in initial data and parameters
      ...
      if (nr.eq.1) then
       ...
       open(10,file='perp.in', status='old')
...
!Initial condition of a gaussian of plasma that slowly spreads out.
!       open(101,file='spbombsmall.in', status='old')
!        do l=1,lt
!        read(101,101)(spbomb(l,m),m=1,mt)
!        end do 
!       print*,'spbomb=', spbomb(10,10)
!       do i = 0,1     
!         ee(1:lt,1:lt,i) = (1.0d0+1.d-3*spbomb(1:lt,1:lt))
!       end do
! 101    format(300(f8.6)) 
...
* call the driving term and update the dummy points:
...
* overwrite any time-dependent boundary conditions:
      do m=m1,m2
        ...
        if (n.ge.noff) then
!          pp(0,m,0)=p0*zterm*xterm
!          qq(0,m,0)=q0*zterm*xterm
! 
!          gg(0,m,0)=g0*zterm*xterm
!          hh(0,m,0)=h0*zterm*xterm
!          dd(0,m,0)=1.d0+d0*zterm*xterm
!          ee(0,m,0)=1.d0+e0*zterm*xterm
        else
         pp(1,m,0)=pp(1,m,0)+p0*zterm*xterm
         qq(1,m,0)=qq(1,m,0)+q0*zterm*xterm

         gg(1,m,0)=gg(1,m,0)+g0*zterm*xterm
         hh(1,m,0)=hh(1,m,0)+h0*zterm*xterm
         dd(1,m,0)=dd(1,m,0)+d0*xterm*zterm
         ee(1,m,0)=ee(1,m,0)+e0*xterm*zterm
        end if
       end do
! 
...
! 	  gg(97,101,0) = -0.9487*g0*decay
! 	  hh(97,101,0) = 0.3163*h0*decay
!  
!  	  gg(97,99,0) = -0.9487*g0*decay
!     	  hh(97,99,0) = -0.3163*h0*decay
! 
! 	  gg(99,103,0) = -0.3163*g0*decay
! 	  hh(99,103,0) = 0.9487*h0*decay
! 
! 	  gg(101,103,0) = 0.3163*g0*decay
! 	  hh(101,103,0) = 0.9487*h0*decay
! 
! 	  gg(103,101,0) = 0.9487*g0*decay
! 	  hh(103,101,0) = 0.3163*h0*decay
! 
! 	  gg(103,99,0)  = 0.9487*g0*decay
! 	  hh(103,99,0)  =-0.3163*h0*decay
! 
! 	  gg(101,97,0)  = 0.3163*g0*decay
! 	  hh(101,97,0)  =-0.9487*h0*decay
! 
! 	  gg(99,97,0)   =-0.3163*g0*decay
! 	  hh(99,97,0)   =-0.9487*h0*decay
! *finally the 45 degree points these mix equally between x and z
! 	  gg(98,102,0)  = -0.7071*g0*decay
! 	  hh(98,102,0)  =  0.7071*h0*decay
! 	 
! 	  gg(102,102,0)  = 0.7071*g0*decay
! 	  hh(102,102,0)  = 0.7071*h0*decay
! 	 
! 	  gg(102,98,0)  =  0.7071*g0*decay
! 	  hh(102,98,0)  = -0.7071*h0*decay
! 	 
! 	  gg(98,98,0)  = -0.7071*g0*decay
! 	  hh(98,98,0)  = -0.7071*h0*decay
!	end if
      end
*---------------------------------------
! 
...
!the l=0 case
        pp(0,m,0)=pp(lt-2,m,0)+3.d0*pp(lt,m,0)-3.d0*pp(lt-1,m,0)
        qq(0,m,0)=qq(lt-2,m,0)+3.d0*qq(lt,m,0)-3.d0*qq(lt-1,m,0)
        mm(0,m,0)=mm(lt-2,m,0)+3.d0*mm(lt,m,0)-3.d0*mm(lt-1,m,0)
        nn(0,m,0)=nn(lt-2,m,0)+3.d0*nn(lt,m,0)-3.d0*nn(lt-1,m,0)
        gg(0,m,0)=gg(lt-2,m,0)+3.d0*gg(lt,m,0)-3.d0*gg(lt-1,m,0)
        hh(0,m,0)=hh(lt-2,m,0)+3.d0*hh(lt,m,0)-3.d0*hh(lt-1,m,0)
        dd(0,m,0)=dd(lt-2,m,0)+3.d0*dd(lt,m,0)-3.d0*dd(lt-1,m,0)
        ee(0,m,0)=ee(lt-2,m,0)+3.d0*ee(lt,m,0)-3.d0*ee(lt-1,m,0)
       end do
C m=0 case:
       do l=1,lt
...
!        do m=0,mt
! !Cl=lt+1 case:
!         pp(lt+1,m,0)=pp(lt,m,0)
!         qq(lt+1,m,0)=qq(lt,m,0)
!         mm(lt+1,m,0)=mm(lt,m,0)
!         nn(lt+1,m,0)=nn(lt,m,0)
!         gg(lt+1,m,0)=gg(lt,m,0)
!         hh(lt+1,m,0)=hh(lt,m,0)
!         dd(lt+1,m,0)=dd(lt,m,0)
!         ee(lt+1,m,0)=ee(lt,m,0)
!        end do
! C m=0 case:
!        do l=1,lt
!         pp(l,0,0)=pp(l,1,0)
...
! !C finally, the dummy points at the far end: m=mt+1
!        do l=1,lt
!         pp(l,mt+1,0)=pp(l,mt,0)
!         qq(l,mt+1,0)=qq(l,mt,0)
!         mm(l,mt+1,0)=mm(l,mt,0)
!         nn(l,mt+1,0)=nn(l,mt,0)
!         gg(l,mt+1,0)=gg(l,mt,0)
!         hh(l,mt+1,0)=hh(l,mt,0)
!         dd(l,mt+1,0)=dd(l,mt,0)
!         ee(l,mt+1,0)=ee(l,mt,0)
!        end do
! 
! !C for corner points (l=lt+1,m=mt+1) and (l=lt+1,m=0) need to extrapolate using the
! !C last row of dummy points at l=lt+1:
