...
* opens all i/o files, reads in initial data and parameters
      ...
      if (nr.eq.1) then
       ...
       open(10,file='sphere.in', status='old')
...
* loop over the time integration
      ...
      do n=1,nt
       if(n.gt.(noff+100).and.n.lt.1100) then
...
C calculate if energy present > threshold
	   ...
	   if(ionise.eq.1) then
	    ee(l,m,1) =  ee(l,m,1) + ioni
            if(l.eq.4.and.m.eq.100)print*,'ionising', ioni, vmag
...
!         msum=m2+m1
! 
!         zterm=dcos(dble(n-1)*pi2*fr/noff)
! 
!         xterm=1.0d0
! 
!         if (n.ge.noff) then
...
! ! l=0
!        do m=0,mt
!         pp(0,m,0)=pp(3,m,0)+3.d0*pp(1,m,0)-3.d0*pp(2,m,0)
!         qq(0,m,0)=qq(3,m,0)+3.d0*qq(1,m,0)-3.d0*qq(2,m,0)
!         mm(0,m,0)=mm(3,m,0)+3.d0*mm(1,m,0)-3.d0*mm(2,m,0)
!         nn(0,m,0)=nn(3,m,0)+3.d0*nn(1,m,0)-3.d0*nn(2,m,0)
!         gg(0,m,0)=gg(3,m,0)+3.d0*gg(1,m,0)-3.d0*gg(2,m,0)
!         hh(0,m,0)=hh(3,m,0)+3.d0*hh(1,m,0)-3.d0*hh(2,m,0)
!         dd(0,m,0)=dd(3,m,0)+3.d0*dd(1,m,0)-3.d0*dd(2,m,0)
!         ee(0,m,0)=ee(3,m,0)+3.d0*ee(1,m,0)-3.d0*ee(2,m,0)
!        end do
!        do m=0,mt
! Cl=lt+1 case:
