...
* opens all i/o files, reads in initial data and parameters
      ...
      if (nr.eq.1) then
       ...
       open(10,file='collide.in', status='old')
...
* code for using input files as initial values
      open(101,file='spbomb.in', status='old')
      do l=1,lt
      read(101,101)(spbomb(l,m),m=0,mt)
      end do


   
      dd(1:200,1:200,0) = (1.0d0+spbomb(1:200,1:200))
101   format(300f6.4) 
...
C calculate if energy present > threshold
 	   ...
 	   ioni = ((ener-emin)*frac)
           totali=totali+ioni
	   if(l.eq.10.and.m.eq.10)print*,'ionising', ioni, vmag
	   if(ionise.eq.1) then
...
!      	if (n.lt.noff) then
!          do m = 0,lt
!           hh(0,m,0) = h0*dcos(n/freq)
!           hh(lt,m,0) = -1.0d0*h0*dcos(n/freq)
!          end do
!         end if
!     
