...
C       do l = 1,lt
C	do m =  1,mt
C	tt = chi*(ee(l,m,1)*pdense)**(2.0/3.0)
C
C	ir = ionrt(tt)

C	ioni = ir*(pdense)*ee(l,m,1)*gdense*dd(l,m,1)*tstep
C	reco = rr*(pdense**2)*(ee(l,m,1)**2)*tstep

C	if((l.eq.50).and.(m.eq.50))print*,ee(l,m,1),(ioni-reco)/pdense

C add alfven ionisation stuff here, for now NOT ENERGY CONSERVING
C	
C	ee(l,m,1)= (ee(l,m,1)*pdense+ioni-reco)/pdense
C	dd(l,m,1)= (dd(l,m,1)*gdense-ioni+reco)/gdense
